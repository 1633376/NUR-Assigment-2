...
def generate_matrix_3D(size, min_distance, func, random_numbers, power):
    """
        Generate a 3D matrix with complex numbers in 
        shifted fourier coordinates using the power spectrum
    In:
        param: size -- The size of the matrix (size x size).
        param: min_distance -- The physical size of 1 cell.
        ...
        param: power -- The power of the power spectrum to create the matrix for.
    Out:
        return: A 3D matrix with complex numbers assigned by the power spectrum
                in fourier shifted coordinates.
    """
    ...
    wavenumber = gen_wavenumbers(size,min_distance)
    # The matrix to return
    ret = np.zeros((size,size,size),dtype=complex)
    ...
    for u in range(size):
        for i in range(size):
            for j in range(size):
                ...
                # Element of k_0,k_0,k_0 is left zero.
                if i == 0 and j == 0 and u ==0:
                    continue
            
                # Calculate the magnitude of the wavenumbers.
                k = np.sqrt(wavenumber[i]**2 + wavenumber[j]**2 + wavenumber[u]**2)
                # Fill the matrix.
                ret[u][i][j] = func(k, power, 
                            random_numbers[steps], random_numbers[steps+1])
                steps += 2
...
def make_hermitian3D(tensor):
    """
        Give a 3D matrix (tensor) in shifted fourier coordinates
        the correct hermitian symmetry so that the ifft is real.
    In:
        param: tensor -- The tensor to give the correct symmetry.
    Out:
        return: A tensor with the correct hermitan symmetry so that the 
                ifft is real.
    """

    # Note: I am fully aware that the loops below can be combined.
    #       The way it is now is however the first way in which I managed
    #       to get it correct and is also the way in which it is easyer to follow.
